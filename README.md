...
### 2. 错误修复与用户体验改进 (2025-05-30，已完成)

*   **中文文件名和日期格式处理**:
    *   修复了知识库中包含中文文件名或特殊格式（如"中越经贸合作"、"日报-生财风格"）的文件无法正常打开的问题。
    *   改进了文件名编码机制，使用`encodeURIComponent`正确处理UTF-8编码字符，替代了原有的正则表达式替换方案。
    *   为保证向后兼容性，添加了多种文件名匹配模式，确保历史上传的文件仍能正常访问。
*   **密码保护优化**:
    *   创建了新的直接访问API端点，提升了文件访问性能和可靠性。
    *   优化了密码验证机制，确保知识库密码保护功能正确工作。
    *   改进了密码输入表单UI，使用微信风格绿色提交按钮，默认将密码输入框设置为明文显示，并保留切换为密码模式的选项。
    *   优化了postMessage机制，确保在同一知识库中浏览不同文件时无需重复输入密码。
